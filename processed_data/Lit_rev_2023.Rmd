...
Q_standard_deviation <- sd(question_table$Mean, na.rm = TRUE)
print(paste("Standard Deviation: ", Q_standard_deviation))

# Calculate mean of Mean
mean_value <- mean(question_table$Mean, na.rm = TRUE)
print(mean_value)

# Calculate values two SDs away from the mean
...
top_performing <- subset(question_table, Mean > top_performing_threshold)
bottom_performing <- subset(question_table, Mean < bottom_performing_threshold)

# Create gt table for question_top and adjust the font size
gt_question_top <- gt(top_performing) %>% 
  tab_options(table.font.size = px(15)
  )
...
print("Bottom-Performing Topics")
print(bottom_performing)

# Create gt table for question_bottom and adjust the font size
gt_question_bottom <- gt(bottom_performing) %>%
  tab_options(
    table.font.size = px(15)
  )
...
```{r}
get_summary_stats(question_table)
